...
import io.github.mooy1.gridutils.implementation.containers.consumers.AbstractGridConsumer;
...
public abstract class AbstractAutoCrafter extends AbstractGridConsumer {
    ...
    private final Map<Location, Pair<RecipeFilter, ItemStack>> cache = new HashMap<>();
    ...
    public final void onNewInstance(@Nonnull BlockMenu menu, @Nonnull Block b, @Nonnull PowerGrid grid) {
        super.instanceGrid(menu, b, grid);
    ...
    @Nonnull
    @Override
    protected final int[] getTransportSlots(@Nonnull DirtyChestMenu menu, @Nonnull ItemTransportFlow flow, @Nonnull ItemStack item) {
        if (flow == ItemTransportFlow.WITHDRAW) {
            return outputSlots;
        }
        @Nullable Pair<RecipeFilter, ItemStack> pair = this.cache.get(((BlockMenu) menu).getLocation());
        if (pair != null) {
    ...
    public final void process(@Nonnull BlockMenu menu, @Nonnull Block b, int tier) {
        ...
        Pair<RecipeFilter, ItemStack> pair = this.cache.get(b.getLocation());
    ...
    @Nullable
    protected abstract Pair<RecipeFilter, ItemStack> getRecipe(@Nonnull ItemStack item);
