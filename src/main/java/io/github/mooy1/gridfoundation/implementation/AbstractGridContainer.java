...
import io.github.mooy1.gridfoundation.implementation.powergrid.interfaces.PowerGridBlock;
...
public abstract class AbstractGridContainer extends AbstractContainer implements PowerGridBlock {
    ...
    public AbstractGridContainer(Category category, SlimefunItemStack item, RecipeType recipeType, ItemStack[] recipe) {
        ...
        addItemHandler((BlockBreakHandler) (e, item12, fortune, drops) -> {
            Block b = e.getBlock();
            PowerGrid grid = removeComponent(b.getLocation());
    ...
    public abstract void onBreak(@Nonnull BlockBreakEvent e, @Nonnull Location l, @Nonnull BlockMenu menu, @Nonnull PowerGrid grid);

    @Override
    public final void onNewInstance(@Nonnull BlockMenu menu, @Nonnull Block b) {
        PowerGrid grid = addComponent(b.getLocation());
        if (grid != null) {
    ...
    public final void tick(@Nonnull Block block, @Nonnull BlockMenu menu) {
        PowerGrid grid = getComponent(block.getLocation());
        if (grid != null) {
